...
# (username, password, login_complete, is_logged_in) shared-session contract.
...
def run_bots():
    ...
    try:
        ...
        print("\n>>> Launching Chrome with remote debugging port 9223...")
        chrome_process = launch_chrome()
        print(">>> Chrome launched successfully. Bots will connect to it now.\n")
        ...
        while True:
            cycle += 1
            print(f"\n{'='*60}")
            print(f"CYCLE {cycle} - Starting {len(targets)} bot(s)")
            print(f"{'='*60}")
            for i, target in enumerate(targets):
                is_logged_in = not (cycle == 1 and i == 0)
                name = target.__name__.replace('start_', '')
                login_mode = 'reusing session' if is_logged_in else 'logging in (FRESH)'
                print(f"\n[{name}] Starting bot ({i+1}/{len(targets)}) - {login_mode}")
                try:
                    target(username, password, login_complete, is_logged_in)
                    print(f"[{name}] [OK] Completed successfully")
                except Exception as e:
            ...
            print(f"\n{'='*60}")
            print(f"CYCLE {cycle} COMPLETE - All bots finished this pass")
            print(f"{'='*60}")
            print(f"No more cases this pass. Sleeping {sleep_seconds}s for new cases...")
